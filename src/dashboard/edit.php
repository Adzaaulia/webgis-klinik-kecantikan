...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($_FILES['image']['name'] != '') {
        ...
        $query = "UPDATE `markers` SET `name` = '$name', `latitude` = '$latitude', `longitude` = '$longitude', `description` = '$description', `image` = '$image' WHERE `markers`.`id` = $id";
    } else {
        $image = $result['image'];
        $query = "UPDATE `markers` SET `name` = '$name', `latitude` = '$latitude', `longitude` = '$longitude', `description` = '$description', `image` = '$image' WHERE `markers`.`id` = $id";
    }
    ...
    if ($result) {
        // get the current url 
        // add params success
        header("Location: /tugas-sig/src/dashboard/index.php?success=true");
    } else {
        // add params error
        header("Location: /tugas-sig/src/dashboard/index.php?error=true");
    }
}
?>
<!DOCTYPE html>
...
<body class="font-inter">
                ...
                <div class="hidden sm:block">
                    ...
                    <div class="relative min-w-72 md:min-w-80">
                        ...
                        <input type="text" id="icon" name="icon"
                            class="py-2 px-4 ps-11 block w-full border-gray-200 rounded-lg text-sm focus:border-purple-500 focus:ring-purple-500 disabled:opacity-50 disabled:pointer-events-none"
                            placeholder="Search">
                    </div>
                </div>
                    ...
                    <div class="hs-dropdown [--placement:bottom-right] relative inline-flex">
                        <button id="hs-dropdown-with-header" type="button"
                            ...
                            <img class="inline-block size-[38px] rounded-full ring-2 ring-white"
                                src="/tugas-sig/src/assets/images/profile.JPG" alt="Image Description"></button>
                        ...
                        <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-60 bg-white shadow-md rounded-lg p-2"
                            ...
                            <div class="mt-2 py-2 first:pt-0 last:pb-0">
                                <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-purple-500"
                                    href="#">
        ...
        <div class="flex justify-between items-center py-2">
            ...
            <ol class="ms-3 flex items-center whitespace-nowrap">
                ...
                <li class="text-sm font-semibold text-gray-800 truncate" aria-current="page">
                    <a href="/tugas-sig/src/dashboard">
                        Dashboard
                    </a>
                </li>
                <li class="text-sm font-semibold text-gray-800 truncate" aria-current="page">
        ...
        <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
            <div class="flex gap-x-4 justify-center">
                <form enctype="multipart/form-data" action="" method="post"
                    class="w-1/3 h-fit p-4 flex flex-col gap-y-4 border border-gray-900 rounded-xl">
                    <div>
                    ...
                    <div class="max-w-sm">
                        ...
                        <input value="<?= $result["name"]; ?>" type="text" id="name" name="name"
                            class="border-5 py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-purple-500 focus:ring-purple-500 disabled:opacity-50 disabled:pointer-events-none"
                            placeholder="Nama">
                    </div>

                    <div class="max-w-sm flex items-center gap-x-4">
                        <div>
                            <label for="lat" class="block text-sm font-medium mb-2">Latitude</label>
                            <input value="<?= $result["latitude"]; ?>" type="text" id="lat" name="lat"
                                class="border-2 py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-purple-500 focus:ring-purple-500 disabled:opacity-50 disabled:pointer-events-none"
                                placeholder="Latitude">
                        </div>
                        <div>
                            <label for="lng" class="block text-sm font-medium mb-2">Longitude</label>
                            <input value="<?= $result["longitude"]; ?>" type="text" id="lng" name="lng"
                                class="border-2 py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-purple-500 focus:ring-purple-500 disabled:opacity-50 disabled:pointer-events-none"
                    ...
                    <div class="max-w-sm">
                        <label class="block">
                            ...
                            <input type="file" name="image"
                                class="block w-full text-sm text-gray-500 file:me-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-purple-600 file:text-white hover:file:bg-purple-700 file:disabled:opacity-50 file:disabled:pointer-events-none">
                        </label>
                    </div>
                    <div>
                        <button type="submit" class="w-full py-3 text-center rounded-xl bg-purple-600 text-white">
                            Submit
                        </button>
                    </div>
                </form>
                <div class="flex-1 h-screen" id="mapid">
    ...
    var map = L.map('mapid').setView([-3.9848743, 122.4546424], 12);
